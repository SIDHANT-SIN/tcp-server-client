...
void serve_file(int newsockfd, const char *file_path) {
    ...
    snprintf(header, sizeof(header), "HTTP/1.1 200 OK\r\nContent-Type: %s\r\nContent-Length: %ld\r\n\r\n",
             get_file_type(file_path), len);
    write(newsockfd, header, strlen(header));

    char buf[1024];
    size_t n;
    while ((n = fread(buf, 1, sizeof(buf), fp)) > 0) write(newsockfd, buf, n);
    fclose(fp);
}
...
void *handle_client(void *arg) {
    ...
    char buf[30000]; bzero(buf, sizeof(buf));
    int n = read(newsockfd, buf, sizeof(buf) - 1);
    if (n < 0) { perror("Error reading"); close(newsockfd); pthread_exit(NULL); }
    buf[n] = '\0';
    ...
    if (strstr(path, "..")) {
        ...
        write(newsockfd, resp, strlen(resp));
        close(newsockfd); pthread_exit(NULL);
    }
    ...
    if (strcmp(method, "GET") == 0) serve_file(newsockfd, file_path);
        ...
        if (body) {
            ...
            if (fp) { fprintf(fp, "%s", body); fclose(fp); pthread_mutex_unlock(&file_mutex);
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        *newsockfd = accept(sockfd, (struct sockaddr *)&cli_addr, &clilen);
        if (*newsockfd < 0) { perror("Accept error"); free(newsockfd); continue; }

        pthread_t tid; pthread_create(&tid, NULL, handle_client, newsockfd);
        pthread_detach(tid);
    }

    close(sockfd); pthread_mutex_destroy(&file_mutex);
    return 0;
}
